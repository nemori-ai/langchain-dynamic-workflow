...
- **Large fan-out batch map** — `ctx.batch_map` streams an `Iterable` / `AsyncIterable` through a bounded admission window, so thousands of items never materialize thousands of tasks; results return in input order with automatic live count/ETA progress.
- **Background-run live drill-in** — a detached background run is no longer opaque: its runtime events are buffered on its slot and a live turn replays them as the same per-leaf cards an inline run streams, so a host can drill into a running background job and report back each run's result on demand.
...
The 19 offline feature demos (one mechanism each, no API key) and the full taxonomy live in **[`examples/AGENTS.md`](examples/AGENTS.md)**.
...
- **Primitives** — exposed on the `Ctx` handed to your script: `agent` / `parallel` / `pipeline` / `race` / `dag` / `loop_until` / `batch_map` / `phase` / `log` / `budget` / `workflow` / `checkpoint`. `ctx.dag(nodes)` runs a `DagNode` dependency graph in topological order (independent-failure tracking); `ctx.loop_until(...)` is a bounded sequential loop helper. `ctx.batch_map(items, fn, *, max_in_flight=None, total=None, label="batch_map")` maps one async `fn` over an `Iterable` / `AsyncIterable` through a bounded admission window (in-flight ≈ `worker_count + queue_maxsize`, decoupled from N), collecting `list[T | None]` in input order (a failing `fn` lands `None`, no abort) and auto-emitting transient count/ETA progress. `ctx.race(candidates, *, win, win_tag="")` runs `RaceCandidate` specs concurrently and returns a `RaceResult` for the first whose result satisfies `win`, cancelling the rest; the decision is journaled (content-hash, `win_tag`-keyed) so resume reproduces the winner and dispatches nothing. `ctx.checkpoint(ask, *, tag="")` pauses the run for a human sign-off — a journal-driven gate that raises `WorkflowSignoffRequired`; the host resumes with the decision via `run_workflow(resume=...)` (the `approve` tool command on the host surface).
...
- **Live observability sinks** — `run_workflow`'s keyword-only `on_span` / `on_span_begin` / `on_leaf_event` / `on_command` (all default no-op, out-of-band) plus the value types `Span` / `SpanBegin` / `SpanKind` / `LeafEvent` / `CommandEvent` and their `*Sink` aliases. For a **detached background run** the same sinks feed a bounded per-slot buffer — `BgRunManager.event_sinks(run_id)` (built from `RunEventSinks`, threaded into the detached `run_workflow`) captures the run's events as `BufferedEvent`s, and `BgRunManager.buffered_events(run_id)` returns them for a live turn to replay; the buffer is transient telemetry (never journaled / replayed, isolation preserved).
- **Topological fan-out + nesting** — `DagNode` (the `ctx.dag` node spec, also injected into the `run_script` namespace) and the `WorkflowDagError` / `WorkflowCycleError` exceptions.
- **Batch progress** — `BatchMetrics` (the `ctx.batch_map` count/ETA payload) and `ProgressKind.BATCH` / `ProgressEntry.metrics`.
- **Types and errors** — `Budget`, `JournalStore` / `InMemoryJournalStore` / `JournalRecord` / `race_key`, the `BgRunManager` family (`RunSnapshot`, `BufferedEvent`, `RunEventSinks`), and the `Workflow*Error` exceptions (`WorkflowScriptError`, `WorkflowSignoffRequired`, `WorkflowCheckpointError`, and the rest).
...
**0.3.0 released (tagged `v0.3.0`); 0.4.0 in progress — not yet published to PyPI.** All three layers are implemented and the public API is stable. The v0.3.0 line landed cross-leaf reduce (M1), race (M2), parallel-run observability (M3.5), cross-session persistence (M3), real local execution (M5), in-run human sign-off (M4), real git worktree + branch/PR (M6), and topological/DAG fan-out + deep nesting (M7). The v0.4.0 line has landed batch ergonomics (`ctx.batch_map`, E) and background-run transport — live drill-in + result report-back (v0.4.0 M3). See [`CHANGELOG.md`](CHANGELOG.md) for the full log.
