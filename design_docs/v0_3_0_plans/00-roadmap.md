...
第一梯队（纯编排层、信号最密、接 v0.2.0 成果）
  M1  F 跨叶归约              ← 首刀；接 G1+G4，无新基建
   └─ M2  B 早退/取消/流式(+E) ← 修核心原语缺陷 + 批处理人体工学
第二梯队（超集赢面、底座已撑、暴露接缝）
...
### M2 · B — 早退 / 取消 / 流式（+ E 批处理人体工学）

**目标：** 给核心调度器补三件事——first-strong-result-wins、一旦某结果确认就**取消在飞兄弟 leaf**、结果**流式产出**（边完成边消费）；顺带补 E 的批处理人体工学（batch-map helper + 自动 count/ETA 进度 + 流式 admission 修急切物化）。

**为何配对：** B 与 E 都改动同一片表面（`parallel`/`pipeline` 调度 + 并发 admission），天然成一个里程碑。

**对比证据（现状）：** `parallel` 严格 all-settle barrier（`_context.py:524-526`）；`pipeline` 阶段间无 barrier 但仍终端 gather、无增量 yield/早退（`_pipeline.py:163-183`）；无在飞兄弟取消，只有整 run cancel（`_background.py:451-476`）。E：并发上限 16/1000 与 CC 一致（`_concurrency.py:30-34`）、动态 N + 背压均работ，但缺 batch-map helper 与自动进度，且 `parallel` 急切物化全部协程（Med 置信隐患）。

**范围：** `race`/`first_completed` 语义或早退回调；在飞 leaf 取消传播；流式结果通道（生成器/回调）；`map`/`batch` 人体工学 helper；count/ETA 进度（扩 `ProgressSink`）；流式 admission（避免 N 千项急切物化）。

**验收门：** #13 AI-SRE 多假设并行 + 早退（一假设确认即取消其余）为真模型 E2E；大批量 fan-out 进度上报作集成示例。

**依赖：** 核心调度器（`_context.py`/`_pipeline.py`/`_concurrency.py`）。
