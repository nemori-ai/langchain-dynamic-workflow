...
│ agent/parallel/pipeline/race/dag/loop_until/phase/log/budget/workflow       │
...
| **④ primitives** | 注入 `agent/parallel/pipeline/race/dag/loop_until/phase/log/budget/workflow`(手写脚本用);`agent` 支持 `schema=`(pydantic 类或 JSON-schema dict)产校验过的结构化对象;`race` 是 best-of-N 早退原语;`dag` 是依赖序(拓扑序)fan-out 原语(`DagNode` 声明依赖边、调度器 Kahn 入度、失败节点传递 skip);`loop_until` 是有终止保证的顺序循环 helper;`workflow` 支持可配置深度上限(默认 8 层)+ 循环检测 | 开发者 / build-time |
