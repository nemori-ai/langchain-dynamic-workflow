...
│ agent/parallel/pipeline/batch_map/race/dag/loop_until/phase/log/budget/workflow │
...
| **④ primitives** | 注入 `agent/parallel/pipeline/batch_map/race/dag/loop_until/phase/log/budget/workflow`(手写脚本用);`agent` 支持 `schema=`(pydantic 类或 JSON-schema dict)产校验过的结构化对象;`batch_map` 是流式准入的大扇出 map(吃 `Iterable`/`AsyncIterable`、有界窗口懒消费、自动发 count/ETA 进度);`race` 是 best-of-N 早退原语;`dag` 是依赖序(拓扑序)fan-out 原语(`DagNode` 声明依赖边、调度器 Kahn 入度、失败节点传递 skip);`loop_until` 是有终止保证的顺序循环 helper;`workflow` 支持可配置深度上限(默认 8 层)+ 循环检测 | 开发者 / build-time |
...
> **DAG 值类型同属开发者面**:`DagNode`(`id`/`deps`/`run`)是一个纯值类型(`_dag` 模块),从包根导出供开发者 `import`,并由 `_codegen` 注入 `run_script` 命名空间(L2 脚本禁 import,按名直构造);`ctx.dag` 和 `ctx.loop_until` 是 `Ctx` 方法,无需注入、亦不在 AST gate 的禁用名单上——详见 [01 §2c/2d](01-engine-mechanism.md)。`WorkflowDagError` / `WorkflowCycleError` 从包根导出(与其他控制流信号同级)。

> **`batch_map` 接线(E,Layer-1)**:`ctx.batch_map(items, fn, *, max_in_flight=None, total=None, label="batch_map")` 是面向大规模扇出的薄 map 原语,接线由三块组成——① **新 `Ctx` 方法**(像 `race`/`dag`/`loop_until` 是 `Ctx` 方法,故无需 `run_script` 注入、亦不在 AST gate 禁用名单上;`BatchMetrics` 引擎产、脚本从不构造);② **广义化的 `run_pipeline`**(`_pipeline.py` 的有界 queue 流式调度器吃任意 `Iterable | AsyncIterable`、拆掉三处 `len` 依赖,`batch_map` 即单 stage 的广义 pipeline 运行,`pipeline` 的 `Sequence` 快速路径逐字节保持,`parallel` 完全不动);③ **transient `ProgressKind.BATCH` 进度**(经 `ProgressLog.emit_transient` 把 `BatchMetrics`/`ProgressEntry.metrics` 投递到既有 `on_progress` sink 但**不记录**,故不入 journal/确定性 guard/replay)。从包根导出 `BatchMetrics`;`SpanKind.BATCH` 记 `total`/`admitted_count`/`surviving_count`——机制见 [01 §9b](01-engine-mechanism.md)。host 经既有 `on_progress`(`run_workflow` 已有参数)渲染 `BATCH` 条目为实时进度条;现有 `parallel`/`pipeline`/`ProgressSink` 调用方零受影响。
...
- **后台 run transport(M3,v0.4.0)**:detached 后台 run 不再 UI-dark。`BgRunSlot` 挂一个**有界 raw-event buffer**(`events: list[BufferedEvent]` + `dropped` 计数 + `events_lock`——`on_command` 在 `asyncio.to_thread` worker 上 append,跨线程故须锁);`BgRunManager.event_sinks(run_id, *, thread_id) -> RunEventSinks` 产五条 append-only sink(同步、不 raise、不 block、drop-newest 过 cap),线进 detached 的 `run_workflow` 五个 sink 形参;`buffered_events(run_id) -> (events, dropped)` 取快照。buffer 是 **transient 遥测**——不入 journal/确定性 guard/replay(沿用 E 的 BATCH delivered-not-recorded 先例),随 slot `sweep` 回收,被 `max_buffered_events × max_concurrent_runs` 双重绑定;隔离照旧(事件只进 buffer、绝不进 host LLM 上下文)。消费侧(demo):一条**活的轮询轮**每 tick 全量重放某 run 的 buffer 进一个**绑当轮 emit 的 fresh `UiAdapter`**——稳定 `span_id` 故 SDK reducer 幂等 upsert——渲染成与 inline run 同款 `agent_span`/`fanout_graph`/`phase_timeline` 卡;`run_runs_board` 收尾另返每 run 的 `RunResult` 摘要(+ 按需 fetch handle)喂回 host,守控制流反转。
...
| `batch_map()`(流式准入大扇出 + count/ETA 进度) | 无原语(`Send` 是平坦 map-reduce barrier,`parallel` 急切物化) | **gap** | 广义化既有 `run_pipeline` 吃 `Iterable`/`AsyncIterable`(拆三处 `len` 依赖、dict 保序回收)+ transient `ProgressKind.BATCH`/`BatchMetrics`(delivered-not-recorded)(见 01 §9b) |
| `dag()`(依赖序/拓扑序 fan-out) | 无原语(`Send` 是平坦 map-reduce barrier) | **gap** | 自建 Kahn 调度器(`_dag`),独立失败跟踪 + 控制流信号穿透(见 01 §2c) |
| `workflow()` 深度上限 + 循环检测 | 零强制(同名重入静默递归) | **gap** | 自建 `_WORKFLOW_DEPTH` / `_WORKFLOW_NAME_STACK` contextvar + `WorkflowNestingError`/`WorkflowCycleError`(见 01 §2e) |
...
**净结论**:provided 占多数;自建集中在 **9 项**——pipeline、**DAG 拓扑序调度器(`_dag`,M7)**、journal(success-only)、确定性 guard、**`workflow()` 深度上限 + 循环检测(M7)**、per-leaf identity、SandboxManager、**真本地执行后端(`LocalSubprocessSandbox`,M5)**、**async 后台 tool 执行机制**——加 budget enforcement 与 `max_concurrency` 显式设界两个收口。

## 7. v1 范围与交付切分

- **v1 主形态①**:库 core `run_workflow()`(真相/地基,可独立测/手写)+ tool adapter `create_workflow_tool()` / middleware(agent 接入主形态);两者一等。
- **v1 含**:十原语(含 race 早退/取消 + **dag 依赖序 fan-out(M7)** + **loop_until 顺序循环(M7)**)+ **`workflow()` 可配置深度上限 + 循环检测(M7)** + roster + 叶子契约 + 双维 guard + journal + 确定性 backstop + pipeline + SandboxManager + **async 后台 tool 机制(run/run_script/status/resume/cancel/runs + 通知注入 + 多并行 run quota + 落定刷新的 workflow_runs)** + budget + 显式并发。
- **L2 紧跟(v1 即可起步)**:skills 形态(SKILL.md),不需重组件——host agent 写脚本传入即可跑。
- **v2 backlog**:A2/A3 安全硬化、codegen prompt 工程深化、可观测性的**外接适配**(phase/log/span → LangSmith trace、独立 `*-tracing` 子包、token rollup)、skill 语义检索。**逐叶 live 可观测性 substrate 已落地(M1,Layer-1 能力)**:span begin 边 + 引擎铸的 resume-稳定 `span_id` + `on_leaf_event` 逐叶回调子树 tap(见 §2.2、[01 §2b](01-engine-mechanism.md));剩在 backlog 的是上述 **LangSmith-adapter / 独立 tracing 子包**(把这些带外 sink 桥到外部 tracer)。**跨进程持久化已落地(M3,`SqliteWorkflowStore` 经 `[sqlite]` extra,见 §10)**;postgres 后端是同 `WorkflowRunStore` 协议后的可插拔扩展点。**真本地执行后端已落地(M5,Layer-1 能力)**:`LocalSubprocessSandbox`(per-leaf 临时根 + 有界输出抽干 + 超时进程组杀 + `ExecGate` + `before_execute` 准入 + POSIX best-effort rlimit + 命令可观测性 `on_command`/`CommandEvent` + 诚实非安全-sandbox posture)作可插拔 Layer-1 后端经 `SandboxManager` 的 `sandbox_factory` 接缝交付,默认仍离线 `InMemorySandbox`、stdlib-only 零依赖(见 [01 §8b](01-engine-mechanism.md))。**真 git worktree + 分支/PR 已落地(M6,Layer-1 能力,配对 M5 成 Bun 级 epic)**:`GitWorktreeProvider`(`git worktree add -b leaf/<id>` 真隔离工作树/分支、真 `git diff` 作权威 collect、幂等+异常安全 teardown、`cleanup_all` host 契约)+ `LocalSubprocessSandbox(root=, on_close=)` 让叶沙箱根植真 worktree + `PullRequestProvider`/`LocalPullRequestProvider`(PR 作 host finalization、移出确定性 replay,R1);整合用 merge 叶内一次性 scratch-repo 真 `git merge`(脚本拥有冲突循环、跨叶状态走脚本变量),经 `SandboxManager(git_worktree_provider=)` 接缝交付、默认仍离线(见 [01 §8c](01-engine-mechanism.md))。**DAG 拓扑序 fan-out + 深层命名嵌套已落地(M7,Layer-1 能力)**:`ctx.dag` 依赖序第四扇出帧(`DagNode(id,deps,run)` + `_dag.run_dag` Kahn 调度器 + 独立失败跟踪 + 控制流信号穿透,`SpanKind.DAG`);`ctx.loop_until` 顺序循环 helper(硬 `max_iters` + 全量累积 `done` 谓词);`ctx.workflow()` 可配置深度上限(`DEFAULT_MAX_WORKFLOW_DEPTH=8`,`max_workflow_depth` 经 `run_workflow→Ctx` 线传)+ 循环检测(`WorkflowCycleError`,名称重入即炸);`WorkflowDagError`/`WorkflowCycleError` 并入 `WORKFLOW_CONTROL_FLOW_SIGNALS`;`DagNode`/`WorkflowDagError`/`WorkflowCycleError` 从包根导出;`DagNode` 注入 `run_script` 命名空间(L2 脚本按名直构造);`ctx.dag`/`ctx.loop_until` 是 `Ctx` 方法,不在 AST gate 禁用名单上(见 [01 §2c/2d/2e](01-engine-mechanism.md))。**批处理人体工学已落地(E,v0.4.0,Layer-1 能力)**:`ctx.batch_map(items, fn, *, max_in_flight=, total=, label=)` 流式准入大扇出 map——广义化既有 `run_pipeline` 吃 `Iterable`/`AsyncIterable`(拆三处 `len` 依赖、`dict` 保序回收,在飞 task ≈ `worker_count + queue_maxsize` 与 N 解耦),失败 `fn` 落 `None` 不 abort、结果按输入序回收 `list[T | None]`;自动发 transient count/ETA 进度(`ProgressKind.BATCH` + `BatchMetrics`,经 `ProgressLog.emit_transient` delivered-not-recorded,故不入 journal/确定性 guard/replay、resume re-emitted-not-replayed);`SpanKind.BATCH` 记 `total`/`admitted_count`/`surviving_count`;`BatchMetrics` 包根导出,`parallel`/`pipeline` 公共契约逐字节不动(见 [01 §9b](01-engine-mechanism.md))。**后台 run transport 已落地(M3,v0.4.0,Layer-1 能力)**:detached 后台 run 的运行事件经 `BgRunManager.event_sinks(run_id)` 五条 append-only sink 缓冲在 `BgRunSlot` 的有界 buffer 上(`BufferedEvent`,drop-newest + `dropped`,跨线程锁),`buffered_events(run_id)` 取出供活轮全量重放进 fresh `UiAdapter`(稳定 `span_id` 幂等),故 detached run 的逐叶内部在启动轮结束后仍可观测;buffer transient 不入 journal/replay、随 slot sweep、隔离照旧;结果回传复用 `RunResult`/`ResultStore`(每 run 摘要 + 按需 fetch)。`BufferedEvent`/`RunEventSinks` 从包根导出,`run_workflow` sink 形参不变——detached 路只是开始用它们(见 §3.2)。剩在 backlog 的 **A2/A3 级硬化**——container / cgroup / 网络隔离——其中 container 后端是同工厂接缝后 `[sqlite]` 之后的**下一个 opt-in extra**;真 GitHub PR(`gh`)是 `PullRequestProvider` 协议后的可插拔实现(离线默认 `LocalPullRequestProvider`)。
...
- **单活 run 不变量(进程内)**:`resume` 重派一个 run 前,`BgRunManager` 按规范 `journal_run_id`(谱系键,非 per-`(thread,run_id)` slot)做**同步 check-and-add 占用**(claim-and-check 间无 `await`,单线程 asyncio 下临界区原子,杜绝 TOCTOU 双 resume);终态 settle 才释放(parked/approve 跨人工暂停续持)。故进程内**至多一个 run 在活同一规范 journal + checkpoint thread**——挡住 origin 仍在活(任意 host thread)、终态 origin 但前一 resume-子 run 仍在活、两 resume 并发竞争三类重复,均 loud 拒(`CanonicalRunInFlightError`)。
- **已知边界**:`args` 须 JSON-可序列化(持久 store 经 JSON round-trip);`race()` 无胜者不 journal,跨进程 resume 会重派候选(新成本);跨进程 resume 须用同一 db 文件。**单活 run 占用是进程局部的**——共享同一 store 的**另一活进程**里仍在跑的 origin 在本进程不可见(其规范不在本进程占用集),故那种跨进程 live 重复检测不到;这是有意取舍——跨进程 resume 仅供崩溃恢复(origin 已死),持久跨进程租约列后续(future work)。
