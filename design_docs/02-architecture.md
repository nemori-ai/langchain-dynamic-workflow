...
> **逐叶 live 可观测性 sink(M1,Layer-1,挂 ① 库 core)**:`run_workflow` 在既有 `on_progress`(进度叙事)/ `on_span`(完成 span)之外,新增两个 keyword-only、默认 `None` no-op 的带外 sink——`on_span_begin`(每个原语 span **打开**即发的 running 边,携引擎铸的 resume-稳定 `span_id` + `started_at`,供 live elapsed timer)与 `on_leaf_event`(把叶子自己的回调子树 normalize 成 `LeafEvent`,经 `leaf_span_id` 关联到所属叶、经 `run_id`/`parent_run_id` 重建叶内 run tree),配 `leaf_event_include_payloads: bool` 控制 `detail` 是否带原始 payload(默认 shape-only)。值类型 `SpanBegin` / `LeafEvent`(+ `SpanBeginSink` / `LeafEventSink` 别名)从包根导出供宿主消费;sink 走带外、不进宿主 LLM 上下文,故接不接 sink 宿主上下文逐字节相同——机制见 [01 §2b](01-engine-mechanism.md)。这些正是交互式 demo app 画 live trace(running chip + elapsed timer + 可折叠嵌套子会话)所消费的引擎面。
