...
- **v1 含**:七原语 + roster + 叶子契约 + 双维 guard + journal + 确定性 backstop + pipeline + SandboxManager + **async 后台 tool 机制(run/status/resume/cancel + 通知注入)** + budget + 显式并发。
