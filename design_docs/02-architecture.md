...
- **v2 backlog**:A2/A3 安全硬化、codegen prompt 工程深化、可观测性(phase/log → LangSmith trace、独立 `*-tracing` 子包、token rollup)、skill 语义检索、`agent(isolation="worktree")` 真 git 后端。**跨进程持久化已落地(M3,`SqliteWorkflowStore` 经 `[sqlite]` extra,见 §10)**;postgres 后端是同 `WorkflowRunStore` 协议后的可插拔扩展点。
