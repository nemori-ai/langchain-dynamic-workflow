...
- **跨叶归约** —— 一等公民 helper 折叠多个叶子的结果：`survives`（refute-by-default 投票）、`dedup`、`reconcile`（双盲复核调解）、`corroborate`（裁判团聚合）。
- **按内容哈希续跑** —— success-only 的 journal 在续跑时以零模型成本重放已完成的叶子。
- **跨会话持久** —— 接上可选的 `[sqlite]` store，全新进程即可按 `run_id` 续跑、以零模型成本重放已完成叶子（超越 Claude Code 的"仅同会话续跑"）。
...
- **Layer 1 —— 编排运行时**：核心原语 —— `agent()`、`parallel()`（barrier）、`pipeline()`（无 barrier）、`race()`（best-of-N 早退）、`phase()`、`log()`、`budget`、`workflow()`、`checkpoint()`（运行中人工签核） —— 外加两个 LangGraph 缺失的补丁：**内容哈希 journal**（原生缓存是 index-based）与 **fail-loud 确定性 guard**（原生把确定性当约定而非不变量）。
...
> ## ⚠️ 真实 shell 执行是一项危险的 opt-in——它不是安全沙箱
>
> 默认情况下，执行叶子用离线内存 backend，**不跑任何真实 shell**。你可以让某个 `SandboxManager` opt-in 到真实的本地子进程 backend（`SandboxManager(sandbox_factory=local_subprocess_factory(ExecPolicy(...)))`），它会把每个叶子的 shell 命令跑在一个**私有的 per-leaf 临时目录**里。**它不是安全沙箱——启用前请务必读完本节。**
>
> **命令仍然能做的事——它以你的身份跑在你的主机上：**
> - **经绝对路径读写任意主机路径。** per-leaf 临时目录只约束*默认工作目录*，并不约束可达文件系统。`cat ~/.ssh/id_rsa` 或 `rm -rf /某/路径` 都在可达范围内。
> - **以你这个用户的权限使用网络**——外传、下载、连到你账号能连的任何地方。
> - **消耗主机资源，超出 best-effort 的 POSIX `rlimit`**——这些 limit 是失控保护，不是 cgroup 级的硬封闭。
>
> **它确实保证的（是韧性，不是封闭）：**
> - 每个叶子一个私有临时工作目录——不会误在引擎自己的目录里执行。
> - 引擎文件 API 始终被 root 约束，`..` 越界被拒。
> - 有界的有效超时 + best-effort 进程组杀（POSIX 上 SIGTERM → grace → SIGKILL）。
> - 有界的合并 stdout+stderr 输出（被截断时打标）。
> - 有界的并发执行数。
>
> **它刻意不提供的（延后的 sharp edge）：**
> - 硬文件系统封闭（container / chroot / nsjail / firejail）。
> - 网络出口阻断。
> - cgroup 级的 CPU / 内存 / 进程封闭。
> - 对刻意守护化或逃出进程组的进程的强孤儿清理。
>
> **最大的风险是虚假的安全感。** per-leaf 临时工作目录*看起来*隔离，其实不是。**绝不要用它跑不可信或对抗性的命令。** 那类命令请把引擎跑在进程外隔离 backend（container）后面。
>
> **Windows 更弱：** 没有 `rlimit`，也没有诚实的资源/安全 posture——只有超时、输出上限、临时工作目录、best-effort 终止和并发上限。
...
[`examples/features/`](examples/features/) 下的 **17 个 feature demo** 各演示一个机制，**离线、无需 API key**（确定性 fake model）即可运行。[`examples/flagship/`](examples/flagship/) 下的两个**旗舰**承载真实端到端路径：默认离线，opt-in 后点亮带原生 web search + prompt caching 的真实 OpenRouter 叶子。要跑真实路径，先 `uv sync --group example` 装上示例附加依赖，在本地 `.env` 配好 `OPENROUTER_API_KEY` 与 `LANGSMITH_*`，再设 `LDW_DEMO_REAL_MODEL`（默认 `anthropic/claude-opus-4.8`，设成任意 OpenRouter slug 即可覆盖）。完整分类与每个 demo 的权威索引见 **[`examples/AGENTS.md`](examples/AGENTS.md)**。

| Demo | 演示内容 |
|---|---|
| **[`flagship/deep_research_preset`](examples/flagship/deep_research_preset.py)** | 旗舰（真模型）：host 驱动已注册的 `deep_research` workflow —— 并行检索 → 抽取 → 对抗式验证 → 综述，带原生 web search + prompt caching。 |
| **[`flagship/deep_research_authored`](examples/flagship/deep_research_authored.py)** | 旗舰（真模型）：host **当场手写** deep-research 脚本并经 `run_script` 运行（AST gate happy path），同一套 web-search + caching 叶子栈。 |

17 个离线 feature demo（各一个机制、无需 API key）与完整分类见 **[`examples/AGENTS.md`](examples/AGENTS.md)**。
